...
#Date:-16/07/2019
#Code is for the half part of appendixc
import pandas as pd
...
import math
df=pd.read_excel("appendexc.xlsx", names=['Serialno','Name','D.O.B','Serialno','Name','D.O.B'])
df.drop([0,1,2,3], axis=0, inplace= True)
df.drop(['Pageno.1'],axis=1,inplace=True)
df.to_excel("dbappendexb2.xlsx", index=False,columns=['Serialno.1','Name.1','D.O.B.1',] )
df=pd.read_excel("dbappendexc2.xlsx")
